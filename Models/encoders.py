...
# import vissl
import wandb
from Models.wide_rn import WideResNet
...
# PreparedModel = namedtuple('PreparedModel',['encoder','latent_dim','transformation','transformation_val'])     
...
class PreparedModel:
    ...
    transformation_val: Callable = None
    classifier: nn.Module = None

               
class TorchModelWrapper(nn.Module):  
    def __init__(self, features_exctractor, function: Callable=None, **kwargs) -> None:
        super().__init__() 
        self.function=function  
        self.extractor = features_exctractor
    def forward(self, x, *args, **kwargs):   
        if self.function is None:  
                out = self.extractor(x)
        else:
            try:
                out = self.function(self.extractor, x)
            except:
                out = self.function(x) 
        return out
...
def prepare_BIT_model_timm(name, pretrained=True, fix_batchnorms_encoder=True, models_path=_models_path, device='cuda', hidden_layer=None, *args, **kwargs)->PreparedModel:  
        ...
        model = TorchModelWrapper(model, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder).to(device) 
        ...
        if 'deit' in name and isinstance(out, Tuple):
            ...
            def forward(model, x): 
                if model.training:
                    ...
                    return model(x)
            model = TorchModelWrapper(model, function=forward, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder).to(device) 
        else:
...
def prepare_BIT_model(name, pretrained=True, fix_batchnorms_encoder=True, models_path=_models_path, device='cuda',  *args, **kwargs)->PreparedModel:  
        ...
        #     return TorchModelWrapper(features_exctractor, flatten=False)
        model = TorchModelWrapper(features_exctractor, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder).to(device) 
...
def prepare_vit_model(encoder_name, pretrained=True, vit_out_layer='last_hidden_state', fix_batchnorms_encoder=True, debug=False, device='cuda', hidden_layer=None, *args, **kwargs)->PreparedModel:
    ...
    if was_training:
        model.train()
    # if self.debug:
    #     model = TorchModelWrapper(self.args, nn.Sequential(*[nn.Flatten(1), nn.Linear(32*32*3,512)]))
    model = TorchModelWrapper(model, function=forward, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder, flatten_features=False).to(device) #
    model.to(device)
...
def prepare_dino_model(encoder_name, fix_batchnorms_encoder, device='cuda', *args, **kwargs)->PreparedModel:
    ...
    #     # latent_dim = list(vits.children())[-3][-1].bn3.num_features
    #     vits = nn.Sequential(*list(vits.children()))
    # else:  
    #     # latent_dim = list(vits.children())[-2].weight.shape[0]
    #     vits = nn.Sequential(*list(vits.children()))
    image = torch.Tensor(torch.ones(5,3,32,32)).to(device)
    latent_dim = np.prod(vits(image).shape[1:])
    return PreparedModel(TorchModelWrapper(vits, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder), latent_dim, None, None)
...
def prepare_encoder_torch(model:nn.Module, pretrained=True, fix_batchnorms_encoder=True, *args, **kwargs)->PreparedModel:
    ...
    features_exctractor = TorchModelWrapper(model, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder)
...
def prepare_encoder_torch_resnet(resnet:nn.Module, pretrained=True, device='cuda', fix_batchnorms_encoder=True, *args, **kwargs)->PreparedModel:
    ...
    image = torch.Tensor(torch.ones(5,3,32,32)).to(device)
    features_exctractor = TorchModelWrapper(nn.Sequential(*list(model.children())[:-1]), keep_bn_in_eval_after_freeze=fix_batchnorms_encoder)
    return PreparedModel(features_exctractor,latent_dim, None, None)
...
def prepare_clip_model(encoder_name, device='cuda', fix_batchnorms_encoder=True, *args, **kwargs)->PreparedModel:
    ...
    # model = TorchModelWrapper(model, function=lambda m,inp: m.encode_image(inp), keep_bn_in_eval_after_freeze=self.args.fix_batchnorms_encoder)
    model = TorchModelWrapper(model, function=model.encode_image, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder)
...
def prepare_resnet_cifar(encoder_name, pretrained=False, fix_batchnorms_encoder=False, device='cuda', *args, **kwargs)->PreparedModel:
    ...
    encoder = TorchModelWrapper(encoder, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder).to(device)
...
def prepare_pt_cifar_model(encoder_name, pretrained=False, fix_batchnorms_encoder=False, *args, **kwargs)->PreparedModel: 
    ...
    features_exctractor = TorchModelWrapper(nn.Sequential(*encoder), keep_bn_in_eval_after_freeze=fix_batchnorms_encoder)
    return PreparedModel(features_exctractor, latent_dim, None, None)
...
#     encoder = TorchModelWrapper(nn.Sequential(*encoder), keep_bn_in_eval_after_freeze=fix_batchnorms_encoder)
...
def prepare_swsl(encoder_name, fix_batchnorms_encoder=False, device='cuda', *args, **kwargs):
    ...
    #     print(o.shape)
    latent_dim = np.prod(model(image).shape[1:])
    return PreparedModel(TorchModelWrapper(model, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder), latent_dim, transform.transforms, transform.transforms)
...
def prepare_efficient_net(name, fix_batchnorms_encoder=False, device='cuda', *args, **kwargs):
    ...
    return PreparedModel(TorchModelWrapper(model, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder), latent_dim, transform, transform)
...
def prepare_cnn(encoder_name, device='cuda', input_shape=32, fix_batchnorms_encoder=True, *args, **kwargs)->PreparedModel:
    ...
    return PreparedModel(TorchModelWrapper(nn.Sequential(model), keep_bn_in_eval_after_freeze=fix_batchnorms_encoder), 512, None, None) 

def prepare_model_wrn(encoder_name, device='cuda', input_shape=32, fix_batchnorms_encoder=True, n_layers=10, width_factor=1, droprate=0., *args, **kwargs)->PreparedModel:
    model= WideResNet(n_layers, 10, width_factor, dropRate=droprate)
    out_features = model.fc.weight.shape[1]      
    # classifier = nn.Sequential(model.bn1, model.relu, model.fc)
    # model.relu=nn.Identity()
    # model.bn1=nn.Identity()
    model.fc=nn.Identity()
    # model(torch.rand((32,32,3)))   
    return PreparedModel(TorchModelWrapper(model, keep_bn_in_eval_after_freeze=fix_batchnorms_encoder), out_features, None, None)#, classifier=classifier) 
...
encoders={   
    ...
    'wrn': EncoderTuple(partial(prepare_model_wrn,'ssl_resnet50'),pretrained_options=[False], info_string='WideResnet not pretrained from https://github.com/xternalz/WideResNet-pytorch/blob/master/wideresnet.py', pretrain_dataset='ImageNet1K'),
...
if __name__=='__main__':    
    encoder: PreparedModel = encoders['resnet34']
